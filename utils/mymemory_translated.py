...
class MyMemoryProvider(BaseProvider):
    ...
    def _make_request(self, text):
        ...
        if self.proxies:
            ...
            # print(f"Using proxy: http{proxy}")
            try:
                response = requests.get(self.base_url, params=params, headers=self.headers, proxies={"http": f'http{proxy}', "https": f'https{proxy}'})
        ...
        # Fallback if no proxies or on proxy failure
